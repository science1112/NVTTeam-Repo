...
from nvtpopcorntime.exceptions import Error, Notify
...
class _MetaClass2(_MetaClass):
    ...
    def _torrent_options(cls):
        ...
        kwargs = {
            ...
            '--no-sparse':              'false',
            #'--resume-file':            None,
            '--user-agent':             'torrent2http/1.0.1 libtorrent/1.0.3.0 nvtpopcorntime/%s' %Addon.version,
            #'--state-file':             None,
...
def load_provider(module):
    provider = "nvtpopcorntime.providers.%s" % module
