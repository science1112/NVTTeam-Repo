...
__scriptname__ = "NVT IPTV"
__author__ = "NVTTeam"
__scriptid__ = "plugin.video.nvtiptv"
__version__ = "1.0.1"
...
def notify(header=None, msg='', duration=5000):
    if header is None: header = 'NVT IPTV'
...
def SEARCHLINKS(url):
    ...
    dp.create("Searching NVT IPTV lists","Searching for:",txtfilter)
    html = getHtml(url)
    blogpage = re.compile('<div class="code">(.*?)</div>', re.DOTALL | re.IGNORECASE).findall(html)[0]
    iptvlinks = re.compile("(h[^<]+)", re.DOTALL | re.IGNORECASE).findall(blogpage)
