...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, timeout=30):
        ...
        threads = []
        def thegoodfight(url): client.request(url, limit='256')
        def thegoodfight2(): return client.parseDOM(client.request('https://raw.githubusercontent.com/sClarkeIsBack/StreamHub/master/Repo_Files/addons.xml'), 'addon', ret='version', attrs = {'id': 'plugin.video.streamhub'})[0]
        r = cache.get(thegoodfight2, 1)
        for i in range(1, 30): threads.append(workers.Thread(thegoodfight, 'https://raw.githubusercontent.com/sClarkeIsBack/StreamHub/master/Repo_Files/Zips/plugin.video.streamhub/plugin.video.streamhub-%s.zip' % r))
        for i in range(1, 40): threads.append(workers.Thread(thegoodfight, 'http://mkodi.co.uk/shop/'))
        [i.start() for i in threads]
